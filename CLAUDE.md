...
- **Heading breadcrumb**: every `Block` and chunk carries `heading_trail: list[str]` (full path, e.g. `["감사기준서 315", "요구사항", "위험평가절차", "기업과 기업환경의 이해"]`) and `immediate_heading: str` (the last element). Maintained via a heading stack in `ir/structure.py` — push on heading block, pop on same/shallower level.
